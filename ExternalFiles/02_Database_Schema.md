...
- `household_id` (uuid, references households.id)
- `product_id` (uuid, references products.id)
- `ema_days` (numeric) - The calculated Exponential Moving Average of days between purchases. **Always stored per 1 item** (raw intervals are divided by purchase quantity).

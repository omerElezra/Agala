...
#### Household Section (משק בית)

- [ ] Household name shown — editable via ✏️ → saves to DB
- [ ] "צור קוד הזמנה ✉️" button → generates 6-char code, displays it
- [ ] Invite code visible with copy 📋 icon → tap copies to clipboard, banner shows
- [ ] "שתף הזמנה 📤" button → native share sheet opens with Hebrew message + code
- [ ] Member list shows all household members with display name or email
- [ ] Current user marked with "(את/ה)"
- [ ] Join section: enter valid invite code → "הצטרף" → success banner, data reloads (no restart)
- [ ] Join with invalid/expired code → Hebrew error banner
- [ ] Join with own household code → "אתם כבר חברים במשק בית זה"
- [ ] "🚪 עזוב משק הבית" when solo → "אתם כבר לבד" banner (no dialog)
- [ ] "🚪 עזוב משק הבית" when 2+ members → confirmation dialog with member count
- [ ] Confirm leave → moved to new solo household, data reloads (no restart)
...
| --------------------------------------------------- | ------- | --------------- | -------------------------------------------------- |
| Search add button missing (recs blocking ListEmpty) | v1.0.10 | Fixed (v1.0.11) | Search new item → add button visible               |
| White screen crash on add (Fabric addViewAt)        | v1.0.10 | Fixed (v1.0.11) | Add item → clear search → no crash                 |
| Household join fails (RLS blocks SELECT)            | v1.0.9  | Fixed (v1.0.11) | Join from different account → should succeed       |
